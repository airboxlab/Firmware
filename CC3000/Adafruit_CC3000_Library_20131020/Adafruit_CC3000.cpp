...
Adafruit_CC3000::Adafruit_CC3000()
{
  ...
  //#endif
  //CC3KPrinter = 0; // HACK removed serial capabilities for now
}
...
/**************************************************************************/
/*!
    @brief   resume CC3000 (after stop)
*/
/**************************************************************************/
void Adafruit_CC3000::resume(void)
{
  if (!_initialised)
  {
    return;
...
/**************************************************************************/
#ifndef CC3000_TINY_DRIVER
uint8_t Adafruit_CC3000::getStatus()
{
  if (!_initialised)
  {
...
{
  //int8_t  _key[MAXLENGTHKEY]; // unused
  //uint8_t _ssid[MAXSSID];     // unused
...
Adafruit_CC3000_Client* Adafruit_CC3000::connectTCP(uint32_t destIP, uint16_t destPort)
{
  ...
  return new Adafruit_CC3000_Client(tcp_socket);
}
...
Adafruit_CC3000_Client* Adafruit_CC3000::connectUDP(uint32_t destIP, uint16_t destPort)
{
  ...
  return new Adafruit_CC3000_Client(udp_socket);
}
